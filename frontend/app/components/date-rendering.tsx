...
export function withDeserializedDates<T, P extends keyof T>(
  ...
  for (const prop of propertiesArray) {
    ...
    if (valueAsString instanceof Date) {
      rehydrated = { ...rehydrated, [prop]: valueAsString };
    } else if (typeof valueAsString !== "string") {
      throw Error(`Cannot parse ${prop.toString()} as date: it is ${typeof valueAsString}`);
    }
...
export function LocalDate({ date }: { date: string | Date }): React.JSX.Element {
