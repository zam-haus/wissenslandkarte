...
export default function Project() {
  ...
  const attachments = project.attachments.map((project) => withDeserializedDates(project, "creationDate"));
  ...
  return <main>
    <header>
      ...
      {t("by")}: <ul>
        {allUsers.map(({ username }) => <li key={username}><Link to={`/users/${username}`}>{username}</Link></li>)}
      </ul>
    ...
    <ul>{updates.map((update) => <li key={update.creationDate.valueOf()}>
      <h4>{t("project-update-headline", { date: renderDate(update.creationDate, i18n.language) })}</h4>{update.description}
    </li>)}</ul>
  </main>
...
function AttachmentEntry(props: Attachment) {
  ...
  switch (type) {
    ...
    case "link": return <a href={url}>{text}</a>
    case "image": return <img src={url} alt={text} />
  }
